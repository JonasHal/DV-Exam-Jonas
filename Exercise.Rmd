...
devtools::install_github("mikkelkrogsholm/leafletDK", force=TRUE)
```
...
dc %<>%
  ...
  mutate(
    ...
    dcdTDGrowth=  ifelse(lag(dcr100k,1)!=0,(dcr100k-lag(dcr100k,1)),0)
  ) %>% relocate(dcdTDGrowth)
...
df %>%
  ...
  ggplot(aes(x =date_sample,y= tr7dP100k/1000)) +
  geom_line()+geom_line(aes(x=date_sample,y=dcr7dPer100k),color="red")+theme_bw()

df %>%
  filter(kommune == "Aalborg") %>%
  ggplot(aes(x =tr7dP100k/1000,y= dcr7dPer100k)) +geom_line()+theme_bw()
  geom_line(aes(x=date_sample,y=dcr7dPer100k),color="red")
  
df %>%
  filter(kommune == "Aalborg") %>%
  ggplot(aes(x =tr7dP100k/1000,y= PosTestRate7d)) +geom_line()+theme_bw()
  geom_line(aes(x=date_sample,y=dcr7dPer100k),color="red")+theme_bw()
  geom_line(aes(x=date_sample,y=dcr7dPer100k),color="red")+theme_bw()
```

```{r coronapositiveanimate, include=TRUE}
p <- municipalityDK("population", "kommune", data = dsize, legend=T)
